...
import type { UpstreamConfig, CachedResponse, Env } from '../types/index.js';
...
import { getFromCache, setInCache } from '../cache/redis-cloudflare.js';
import { logRequest } from '../logging/mongodb-cloudflare.js';
import { getConfig } from '../config/loader-cloudflare.js';
...
export async function proxyRequest(
  ...
  const { subdomain, upstream, cleanUrl, env } = ctx;
  ...
  if (matchingRule && method === 'GET') {
    ...
    const cachedResponse = await getFromCache(cacheKey, env);
    ...
    if (cachedResponse) {
      ...
      if (responseStatus >= 200 && responseStatus < 400) {
        ...
        await setInCache(cacheKey, cacheData, matchingRule.ttl, env);
      }
    }
  } else {
    const result = await fetchFromUpstream(c, upstream, cleanUrl, requestBody);
